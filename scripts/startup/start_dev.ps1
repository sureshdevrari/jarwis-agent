...
function Stop-PortProcess {
    ...
    # Method 2: Force kill with taskkill as fallback
    Start-Sleep -Milliseconds 500
    $stillRunning = netstat -ano 2>$null | Select-String ":$Port\s+.*LISTEN"
    if ($stillRunning) {
        Write-Host "  Warning: Port $Port still in use, forcing cleanup..." -ForegroundColor Yellow
        $stillRunning | ForEach-Object {
            $parts = $_ -split '\s+'
            $pid = $parts[-1]
            if ($pid -match '^\d+$') {
                taskkill /F /PID $pid 2>$null | Out-Null
                Write-Host "  Force killed PID $pid" -ForegroundColor Gray
            }
        }
