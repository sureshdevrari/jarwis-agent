...
# Function to kill processes on a specific port (aggressive cleanup)
function Stop-PortProcess {
    ...
    # Method 2: Kill by known process names
    $processNames = @('node', 'python', 'uvicorn')
    ...
    $stillRunning = netstat -ano | Select-String ":$Port.*LISTEN"
    if ($stillRunning) {
        Write-Host "  Warning: Port $Port may still be in use, forcing cleanup..." -ForegroundColor Yellow
        $stillRunning | ForEach-Object {
            $parts = $_ -split '\s+'
            if ($parts[-1] -match '^\d+$') {
                taskkill /F /PID $parts[-1] 2>$null
            }
        }
    }
}
