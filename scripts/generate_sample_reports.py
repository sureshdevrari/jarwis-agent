"""
Generate Sample Jarwis Security Reports
Creates sample reports for Web, Mobile, and Cloud scans with dummy data
"""
...
async def main():
    ...
    for config in report_configs:
        ...
        # Initialize generator with HTML format (will convert to PDF)
        generator = ReportGenerator(str(output_dir), ["html"])
        
        # Generate HTML report first
        try:
            html_files = await generator.generate(
            ...
            if html_files:
                ...
                success = generator.generate_pdf_sync(html_path, pdf_path)
                
                if success:
                    print(f"   [OK] PDF generated: {pdf_path.name}")
                    generated_files.append(str(pdf_path))
                    
                    # Remove the HTML file (keep only PDF)
                    # html_path.unlink()
                else:
